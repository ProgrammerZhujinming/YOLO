# YOLOv1
个人博客YOLOv1算法详解地址：https://blog.csdn.net/qq_39304630/article/details/112394446  
...
5.与身边朋友沟通后认为，作者在文中指出的confidence=Pr(obj)*IoU，指的是让网络对于置信度的回归目标变为当前的bounding box与 ground_truth的IoU,网络依据IoU的值判定含有物体的概率。

# 更新日志 8-10  
1.查阅了资料，正在确定定位损失高权重的迭代周期权重，最终代码将在确定训练方案后发出，然后会进入YOLO V3的复现环节。  
2.由于余弦退火算法是周期性的变动学习率来帮助冲出某些局部最优点，而Adam本身就具有学习率衰减的作用，考虑后认为余弦退火可能更加适合SGD而不适合Adam，因此已经在新版本中取消了余弦退火。  
3.pytorch的随机分割数据集的方法本身就具有随机结果性，因此我们不再手动调用所写的DataSet的shuffle方法。  
4.对于YOLO V1预测的两个box而言，不再默认取拥有最高置信度的那个，而是选择与Ground Truth拥有更大IoU值的那个，选定它对目标位置进行回归。
  
# 更新日志 8-19  
1.除预测层外全部使用ReLU发现到网络后期，置信度损失异常飙高，替换为LeakReLU后目前没有发现此类现象，怀疑是ReLU的大量使用以及网络本身复杂度过高(高于使用的数据集)造成了网络的退化。  
2.对于存在BN层与Drouput层的网络，需要记得在训练时使用model.train方法开启训练，在测试时使用model.eval方法开启验证，因为BN与Drouput在训练与测试时的行为是不一样的。  
  
# 更新日志 8-26
1.BN与Drouput存在冲突，同时使用容易引起偏差，但是BN层很容易加速收敛，因此本人在此舍弃Drouput只用BN。  
2.验证时也需要启动eval模式而不是train模式，同时验证时也不该记录梯度。  
3.变量名修正，表达语义更强。  
...
1.mAP计算脚本设计
2.应用部分重构
3.同时支持分类部分使用mse或cross_entropy
4.提供预训练的backbone模型权重