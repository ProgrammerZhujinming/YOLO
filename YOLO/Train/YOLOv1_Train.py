...
#def train():
    ...
    parser.add_argument('--lr', type=float, help="start lr", default=3e-4)
    ...
    parser.add_argument('--lr_epoch_200', type=int, help="lr when full YOLOv1 train in epoch200", default=0.000003)
    parser.add_argument('--batch_size', type=int, help="YOLOv1 train batch size", default=32)
    parser.add_argument('--train_imgs', type=str, help="YOLOv1 train train_imgs", default="../../DataSet/VOC2007+2012/Train/JPEGImages")
    parser.add_argument('--train_labels', type=str, help="YOLOv1 train train_labels", default="../../DataSet/VOC2007+2012/Train/Annotations")
    parser.add_argument('--val_imgs', type=str, help="YOLOv1 train val_imgs", default="../../DataSet/VOC2007+2012/Val/JPEGImages")
    parser.add_argument('--val_labels', type=str, help="YOLOv1 train val_labels", default="../../DataSet/VOC2007+2012/Val/Annotations")
    parser.add_argument('--voc_classes_path', type=str, help="voc classes path", default="../../DataSet/VOC2007+2012/class.data")
    ...
    parser.add_argument('--epoch_num', type=int, help="YOLOv1 train epoch num", default=300)
    parser.add_argument('--grad_visualize', type=bool, help="YOLOv1 train grad visualize", default=True)
    parser.add_argument('--feature_map_visualize', type=bool, help="YOLOv1 train feature map visualize", default=True)
    parser.add_argument('--restart', type=bool, default=True)
    parser.add_argument('--pre_weight_file', type=str, help="YOLOv1 BackBone pre-train path", default="../PreTrain/weights/YOLO_Feature_180.pth")
    parser.add_argument('--weight_file', type=str, help="YOLOv1 weight path", default="./weights/YOLOv1_200.pth")
    ...
    lr_epoch_200 = args.lr_epoch_200
    batch_size = args.batch_size
    ...
    if args.restart == True:
        YOLO.initialize_weights(pre_param_dict["optimal"]) #load darknet pretrain weight
        ...
        optimizer_SGD = param_dict['optimizer']
    ...
    while epoch < args.epoch_num:
        ...
        YOLO  =  YOLO.train()
        with tqdm(total=train_len) as train_tbar:
            ...
            if args.feature_map_visualize:
                feature_map_visualize(train_data[0], writer, YOLO)
        ...
        YOLO =  YOLO.eval()
        with tqdm(total=val_len) as val_tbar:
            with torch.no_grad():
                ...
                for batch_idx, [val_data, label_data] in enumerate(val_loader):
                    ...
                    val_tbar.set_description(
                        "val: coord_loss:{} pos_conf_loss:{} neg_conf_loss:{} class_loss:{} avg_iou:{}".format(
                            round(loss[1], 4),
                            round(loss[2], 4),
                            round(loss[3], 4),
                            round(loss[4], 4),
                ...
                if args.feature_map_visualize:
                    feature_map_visualize(val_data[0], writer, YOLO)
                # print("batch_index : {} ; batch_loss : {}".format(batch_index, batch_loss))
            print("val-batch-mean loss:{} coord_loss:{} pos_conf_loss:{} neg_conf_loss:{} class_loss:{} iou:{}\n".format(
        ...
        writer.add_scalar('Train/Loss_sum', epoch_train_loss / train_len / batch_size, epoch)
        writer.add_scalar('Train/Loss_coord', epoch_train_loss_coord / train_len / batch_size, epoch)
        writer.add_scalar('Train/Loss_pos_conf', epoch_train_loss_pos_conf / train_len / batch_size, epoch)
        writer.add_scalar('Train/Loss_neg_conf', epoch_train_loss_neg_conf / train_len / batch_size, epoch)
        writer.add_scalar('Train/Loss_classes', epoch_train_loss_classes / train_len / batch_size, epoch)
        writer.add_scalar('Train/iou', epoch_train_iou / epoch_train_object_num, epoch)

        writer.add_scalar('Val/Loss_sum', epoch_val_loss / val_len / batch_size, epoch)
        writer.add_scalar('Val/Loss_coord', epoch_val_loss_coord / val_len / batch_size, epoch)
        writer.add_scalar('Val/Loss_pos_conf', epoch_val_loss_pos_conf / val_len / batch_size, epoch)
        writer.add_scalar('Val/Loss_neg_conf', epoch_val_loss_neg_conf / val_len / batch_size, epoch)
        writer.add_scalar('Val/Loss_classes', epoch_val_loss_classes / val_len / batch_size, epoch)
        writer.add_scalar('Val/iou', epoch_val_iou / epoch_val_object_num, epoch)
